...
const Skiper19 = () => {
  ...
  const textSectionOpacity = useTransform(scrollYProgress, [0.08, 0.12, 0.32, 0.38], [0, 1, 1, 0]);
  ...
  const mobileAppOpacity = useTransform(scrollYProgress, [0.38, 0.44, 0.8, 0.85], [0, 1, 1, 0]);

  return (
    <section
      ref={ref}
      className="mx-auto flex h-[600vh] w-screen flex-col items-center overflow-hidden bg-black px-4 text-white"
    >
      ...
      <motion.div
        className="sticky top-[30vh] z-40 flex flex-col items-center gap-10 mt-[80vh]"
        style={{ opacity: mobileAppOpacity }}
      >
        <div
          className="w-full max-w-4xl text-center text-5xl font-bold uppercase tracking-tighter sm:text-6xl md:text-7xl lg:text-8xl"
          style={{ perspective: "500px" }}
        >
...
const LinePath = ({ className, scrollYProgress }) => {
  const pathLength = useTransform(scrollYProgress, [0, 0.50], [0.5, 1]);
