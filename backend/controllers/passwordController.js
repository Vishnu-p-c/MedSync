...
function buildResetEmailHtml(user, resetLink) {
  return `
...
// POST /password/forgot  (WEB APP — admins only)
...
exports.forgotPassword = async (req, res) => {
  try {
    ...
    // Admin-only check for web app
    if (user.role !== 'admin') {
      return res.json({
        status: 'fail',
        message: 'Password reset on the web app is only available for admins.'
      });
    }
    ...
    await sendEmail({
      ...
      html: buildResetEmailHtml(user, resetLink),
    });
    ...
    return res.json(
        {status: 'success', message: `Password reset link sent to ${masked}`});
  } catch (err) {
...
// POST /password/mobile/forgot  (MOBILE APP — patient, doctor, driver only)
// Accepts { identifier } which can be username or email
exports.mobileForgotPassword = async (req, res) => {
  try {
    const {identifier} = req.body;
    ...
    // Mobile app — only patient, doctor, driver allowed
    if (!['patient', 'doctor', 'driver'].includes(user.role)) {
      return res.json({
        status: 'fail',
        message:
            'This reset is only available for patients, doctors, and drivers.'
      });
    }
