...
exports.getDoctorsCount =
    async (req, res) => {
  try {
    // Get admin_id from request (sent from frontend)
    const admin_id =
        req.query.admin_id || req.body.admin_id || req.params.admin_id;
    ...
    // Note: hospital_id and clinic_id are arrays in Doctor model, so use $in to
    // check if facilityId is in the array
    let doctorsCount;
    if (facilityType === 'hospital') {
      doctorsCount =
          await Doctor.countDocuments({hospital_id: {$in: [facilityId]}});
    } else {
      doctorsCount =
          await Doctor.countDocuments({clinic_id: {$in: [facilityId]}});
    }
    ...
    if (facilityType === 'hospital') {
      availableDoctorsCount = await Doctor.countDocuments(
          {hospital_id: {$in: [facilityId]}, is_available: true});
    } else {
      availableDoctorsCount = await Doctor.countDocuments(
          {clinic_id: {$in: [facilityId]}, is_available: true});
    }
    ...
    return res.status(500).json({
      ...
      error: error.message
                        ...
                        // Controller to get critical alerts (equipment under
                        // maintenance/down and low stock)
  ...
  try {
    ...
    const lowStockAlerts =
        await Stock.find({...hospitalFilter, quantity: {$lte: 10}})
            .sort({quantity: 1})
            .limit(10);
    ...
    equipmentAlerts.forEach(equipment => {
      alerts.push({
        ...
        type: equipment.status === 'down' ? 'Equipment Fault' :
                                            'Under Maintenance',
        item: equipment.equipment_name,
        message: equipment.status === 'down' ? 'Equipment is not operational' :
  ...
  try {
    ...
    if (adminUser) {
      adminInfo = {
        ...
        fullName: `${adminUser.first_name}${
            adminUser.last_name ? ' ' + adminUser.last_name : ''}`,
    ...
    if (adminRecord.admin_type === 'hospital' && adminRecord.hospital_id) {
      const hospital =
          await Hospital.findOne({hospital_id: adminRecord.hospital_id});
      if (hospital) {
        facilityInfo = {
    ...
    return res.json({
      status: 'success',
      data: {...facilityInfo, admin: adminInfo},
      timestamp: new Date()
    });
...
// Controller to get all doctors for an admin's hospital/clinic with search and
// filter
exports.getDoctorsList = async (req, res) => {
  try {
    ...
    if (search) {
      ...
      query.$or = [
        {first_name: searchRegex}, {last_name: searchRegex},
        {name: searchRegex}, {department: searchRegex}, {mrn: searchRegex}
      ];
    }
      ...
      if (doctor.last_attendance_time) {
        ...
        if (diffHours < 24) {
          ...
          lastCheckIn = 'Yesterday, ' +
              attendanceDate.toLocaleTimeString('en-IN', {
                hour: 'numeric',
                minute: '2-digit',
                hour12: true,
                timeZone: 'Asia/Kolkata'
          ...
          // Older - show date
          lastCheckIn = attendanceDate.toLocaleDateString(
              'en-IN', {day: '2-digit', month: '2-digit', year: 'numeric'});
        }
      }
  ...
  try {
    ...
    if (existingUser) {
      ...
      return res.status(409).json(
          {status: 'fail', message: `${conflictField}_already_exists`});
    }
    ...
    try {
      ...
      if (userSaveError.code === 11000) {
        ...
        return res.status(409).json(
            {status: 'fail', message: `${field}_already_exists`});
      }
      throw userSaveError;
    }
