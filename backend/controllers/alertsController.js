...
exports.getAllAlerts = async (req, res) => {
  try {
    ...
    const adminRecord =
        await HospitalAdmin.findOne({admin_id: parseInt(admin_id)});
    ...
    const equipmentAlerts =
        await Equipment
            .find({...hospitalFilter, status: {$in: ['maintenance', 'down']}})
            .sort({last_updated: -1});
    ...
    const lowStockAlerts =
        await Stock.find({...hospitalFilter, quantity: {$lte: 20}}).sort({
          quantity: 1
        });
    ...
    lowStockAlerts.forEach(stock => {
      ...
      alerts.push({
        ...
        message: isCritical ?
            `Critical: Only ${
                stock.quantity} units remaining - Restock immediately!` :
    ...
    try {
      const pendingSos = await SosRequest
                             .find({
    ...
    try {
      const activeAssignments =
          await AmbulanceAssignment
              .find({status: {$in: ['assigned', 'en_route', 'picked_up']}})
              .sort({assigned_at: -1})
              .limit(5);
      ...
      activeAssignments.forEach(assignment => {
        alerts.push({
          ...
          message: `Status: ${
              assignment.status.replace('_', ' ')} - Active emergency response`,
    ...
    // 5. Inter-Hospital Messages (unread messages from other hospitals)
    try {
      if (adminRecord && adminRecord.hospital_id) {
        const unreadMessages =
            await InterHospitalMessage
                .find({to_hospital_id: adminRecord.hospital_id, is_read: false})
                .sort({timestamp: -1})
                .limit(20)
                .lean();

        // Get sender hospital names
        const senderIds =
            [...new Set(unreadMessages.map(m => m.from_hospital_id))];
        const senderHospitals =
            await Hospital
                .find(
                    {hospital_id: {$in: senderIds}}, {hospital_id: 1, name: 1})
                .lean();
        const hospitalNameMap = {};
        senderHospitals.forEach(h => hospitalNameMap[h.hospital_id] = h.name);

        unreadMessages.forEach(msg => {
          const senderName =
              hospitalNameMap[msg.from_hospital_id] || 'Unknown Hospital';
          const priorityType = msg.priority === 'critical' ? 'critical' :
              msg.priority === 'urgent'                    ? 'warning' :
                                                             'info';
          alerts.push({
            id: `ihm-${msg.message_id}`,
            type: priorityType,
            category: 'Hospital Message',
            title: msg.subject || `Message from ${senderName}`,
            message: msg.message.length > 200 ?
                msg.message.substring(0, 200) + '...' :
                msg.message,
            from_hospital_id: msg.from_hospital_id,
            from_hospital_name: senderName,
      ...
      console.log('Inter-hospital message alerts skipped:', e.message);
    }
    ...
    return res.json(
        {status: 'success', data: alerts, summary: summary, timestamp: now});
...
exports.getAlertsSummary = async (req, res) => {
  try {
    ...
    const equipmentCount = await Equipment.countDocuments(
        {...hospitalFilter, status: {$in: ['maintenance', 'down']}});

    // Count low stock items
    const stockCount =
        await Stock.countDocuments({...hospitalFilter, quantity: {$lte: 20}});

    // Count critical items
    const criticalEquipment =
        await Equipment.countDocuments({...hospitalFilter, status: 'down'});

    const criticalStock =
        await Stock.countDocuments({...hospitalFilter, quantity: {$lte: 5}});
